...
export default function ChapterBanner({
  ...
  return (
    <div
      ref={ref}
      className="sticky top-16 z-[55] py-5 sm:py-6 px-4 border-b-4 border-[var(--course-border)]"
      style={{
    ...
    >
      <div className="max-w-3xl mx-auto relative">
        ...
        >
          <span
            className="flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 border-[3px] border-[var(--course-border)] font-heading font-black text-sm sm:text-base text-white flex-shrink-0"
        ...
        {isOpen && (
          <ol
            className="absolute left-0 right-0 top-full mt-1 flex flex-col gap-1 px-4 py-3 border-4 border-[var(--course-border)] z-50 animate-fade-in"
            style={{ backgroundColor: "var(--course-bg)", boxShadow: 'var(--course-shadow-lg)' }}
          >
            {modules.map((mod, i) => {
                  ...
                  >
                    <span
                      className={`text-xs font-black font-heading w-6 h-6 flex items-center justify-center flex-shrink-0 ${
                        isCurrent
                          ? "text-white"
                          : "text-[var(--course-text-muted)] border-2 border-[var(--course-border-muted)]"
                      }`}
                      style={
                        isCurrent
