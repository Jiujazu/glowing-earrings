...
export default function StepByStep({ element }: { element: StepByStepElement }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--course-surface)] border-4 border-[var(--course-border)]" style={{ boxShadow: "var(--course-shadow)" }}>
      ...
      <ol className="divide-y divide-[var(--course-border-muted)]">
        {steps.map((step, i) => {
          ...
          return (
            <li key={i}>
              ...
              >
                <span className="flex items-center justify-center w-8 h-8 border-[3px] border-[var(--course-border)] bg-[var(--course-primary)]/20 text-[var(--course-primary)] text-sm font-black font-heading flex-shrink-0">
                  {i + 1}
                </span>
              ...
              {isExpanded && (
                <div className="px-5 pb-5 pl-[4.25rem] animate-slide-up">
                  ...
                  {step.image && (
                    <div className="mt-3 overflow-hidden border-4 border-[var(--course-border)]">
                      <Image
                        src={step.image}
